...
def parseIdx(filename): 
    ...
    with open(filename ,'rb') as in_file:
        ...
        while True:
            ...
            print("%i: %i"%(idx,offset))               # I also like it all in caps. 
            if(idx > 0):
                parts[idx]={}
...
def play_audio(idx):
    ...
    run( [ 'ffplay', '-hide_banner','-loglevel','warning','-autoexit','-i',file,'-ss',seek,'-t',duration_str ])
    return
...
def play_video(idx):
    ...
    with open(file,'rb') as fin:
        ...
        run( [ 'ffplay', '-hide_banner','-loglevel','warning','-autoexit','-' ], input=fin.read(end - start))
    return
